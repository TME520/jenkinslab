    ...
    stages {
        ...
        stage('Create stack') {
            steps {
                ...
                withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: '411e79d0-00f9-4be4-babb-c26fac151e88', usernameVariable: 'AWS_ACCESS_KEY_ID', passwordVariable: 'AWS_SECRET_ACCESS_KEY']]) { AWS("--region=ap-southeast-2 cloudformation create-stack --stack-name ${params.stack_name} --template-url https://cf-templates-w4ea9ebnhuyx-ap-southeast-2.s3-ap-southeast-2.amazonaws.com/apolloTuto.json --parameters ParameterKey=HostedZoneName,ParameterValue=${params.hosted_zone_name} ParameterKey=SSHLocation,ParameterValue=${params.authorized_ip}") }
            }
        }
